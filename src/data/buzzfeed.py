...
class BuzzFeed(DatasetUsingDataframe):
    def __init__(self) -> None:
        super().__init__()
        buzzfeedfake = pd.read_csv(os.path.join(DIR, '..', '..', 'Data', 'BuzzFeed_fake_news_content.csv'))
        buzzfeedfake['label'] = 1
        buzzfeedfake = buzzfeedfake.drop(columns=['title', 'id', 'url', 'top_img', 'authors', 'source', 'publish_date', 'movies', 'images', 'canonical_link', 'meta_data'])
        buzzfeedreal = pd.read_csv(os.path.join(DIR, '..', '..', 'Data', 'BuzzFeed_fake_news_content.csv'))
        buzzfeedreal['label'] = 0
        buzzfeedreal = buzzfeedreal.drop(columns=['title', 'id', 'url', 'top_img', 'authors', 'source', 'publish_date', 'movies', 'images', 'canonical_link', 'meta_data'])
        df = pd.concat([buzzfeedfake, buzzfeedreal])
